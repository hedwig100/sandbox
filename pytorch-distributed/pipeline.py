...
class ResNetBlock(nn.Module):
    def __init__(
        ...
        self._lock = (
            threading.Lock()
        )  # protect the model from being accessed by multiple machines at the same time
        self._block = block
...
class ResNetShard1(ResNetBlock):
...
class ResNetShard2(ResNetBlock):
    def __init__(self, device, *args, **kwargs):
        ...
        self.fc = nn.Linear(512 * self._block.expansion, NUM_CLASSES).to(
            self.device
        )
