...
export function ChatPanel({ messages, activities, isLoading, error, onSend, onRetry, onRetryMessage }: { messages: ChatMessage[]; activities: ToolActivity[]; isLoading: boolean; error?: string; onSend: (text: string) => void; onRetry: () => void; onRetryMessage?:(id:string)=>void }) {
  ...
  return <section className="chatPanel" aria-label="旅行エージェントとの会話">
    ...
    <div className="conversation" aria-live="polite">
      {messages.length === 0 ? <div className="welcome"><div className="eyebrow"><Compass size={15}/> AI TRAVEL CONCIERGE</div><h1>まだ知らない旅を、<br/><em>会話から。</em></h1><p>行きたい季節、好きなこと、予算。断片的なアイデアから、あなただけの旅を一緒に描きます。</p><div className="starters">{starters.map((starter) => <button key={starter} onClick={() => onSend(starter)}><span>{starter}</span><ArrowUp size={15}/></button>)}</div></div> : messages.map((message) => <div className={`message ${message.role} ${message.status||""}`} key={message.id}><span className="messageLabel">{message.role === "user" ? "YOU" : "CONCIERGE"}{message.createdAt&&` · ${new Date(message.createdAt).toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"})}`}</span><div>{message.content || (message.status==="pending" ? <span className="typing"><i/><i/><i/></span> : message.status==="failed"?<span>回答を完了できませんでした。</span>:null)}</div>{message.activities&&message.activities.length>0&&<details open={message.status==="failed"}><summary>ツール実行 {message.activities.length}件</summary>{message.activities.map(item=><div className="activityRow" key={item.id}><Check size={14}/>{item.label} · {item.status}</div>)}</details>}{message.status==="failed"&&onRetryMessage&&<button className="messageRetry" onClick={()=>onRetryMessage(message.id)}><RotateCcw/>回答を再試行</button>}</div>)}
